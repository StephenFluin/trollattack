...
public class LinkedList {
    ...
    /**
     * Sort type of 0 indicates no sorting Sort type of 1 indicates ascending
     * sort. Sort type of -1 indicates descending sort.
     */
    ...
    private void addNode(Object o) {
        ...
        if (allowDuplicates || !this.lookup(o)) {
            ...
            case 0:
                ...
                break;
            case -1:
                while (current.getPointer().getData() != null
                        && (current.getPointer().getData().toString()
            ...
            case 1:
                while (current.getPointer().getData() != null
                        && current.getPointer().getData().toString().compareTo(
                                o.toString()) < 0) {
                    current = current.getPointer();
                }
                current.setPointer(new Node(o, current.getPointer()));
    ...
    /**
     * searches the List for an Object identical to obj and returns true if
    ...
    public boolean lookup(Object obj) {
        ...
        while (current != tail && !current.getData().equals(obj)) {
            //Lab3.print("Comparing " + obj.toString() +
            // current.getData().toString() + " is " + current.getData().equals(
            // obj ) );
            current = current.getPointer();
        }
        if (current == tail) {
    ...
    /**
     * getClosest Used to find the closest matching command in the list. Such as
     * no should math north as long as north is before nod in the list.
     * 
     * @return Command.
     */
